...
const Signup = () => {
      ...
      <div className="bg-surface-container-low min-h-screen flex items-center justify-center p-gutter pt-16">
        <main className="w-full max-w-md bg-surface-container-lowest rounded-xl border border-sand shadow-ambient-sm p-lg">
          ...
          <form className="space-y-md" onSubmit={handleOtpSubmit}>
            <div>
              <label
                className="block font-label-caps text-label-caps text-on-surface mb-sm"
                htmlFor="otp"
              >
                Verification Code
              </label>
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center p-6 font-body-md text-body-md antialiased relative overflow-hidden transition-colors duration-700 ${theme.bg}`}
    >
      {/* Dynamic Background Decor */}
      <div
        className={`absolute top-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full blur-[120px] animate-pulse transition-colors duration-700 ${theme.glow}`}
      ></div>
      <div
        className={`absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full blur-[120px] animate-pulse transition-colors duration-700 ${theme.glow}`}
      ...
      <main className="w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-100 p-8 overflow-hidden animate-fade-in-up relative z-10 my-4">
        <div className="flex flex-col items-center mb-4">
          ...
          <p className="text-slate-500 text-base text-center font-medium">
            Join as a{" "}
            <span
              className={`font-semibold transition-colors duration-300 ${theme.text}`}
            >
              {role === "recruiter" ? "Company" : role}
            </span>
          </p>
        </div>

        <div
          className="flex p-1 bg-slate-50 rounded-xl mb-6 border border-slate-100 animate-fade-in-up"
        ...
        >
          {[
            ...
            <div key={field.id} className="space-y-1">
              ...
              <div className="relative group">
                <span
                  className={`material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:${theme.text} transition-colors text-2xl`}
                >
                  {field.icon}
                </span>
                <input
          ...
          <div className="flex items-start gap-3 ml-1 pt-1">
            <input
              className="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-600/20 cursor-pointer mt-0.5"
              id="terms"
              type="checkbox"
              required
            />
            <label
              className="text-sm font-medium text-slate-500 cursor-pointer leading-tight"
              htmlFor="terms"
            >
              I agree to the{" "}
              <Link
        ...
        <div
          className="mt-6 pt-6 border-t border-slate-50 text-center animate-fade-in"
          style={{ animationDelay: "0.4s" }}
        >
          <p className="text-sm text-slate-500 font-medium">
            Already have an account?{" "}
            <Link
              className="text-blue-600 font-semibold hover:text-blue-700 hover:underline transition-all ml-1"
              to="/login"
            >
              Log in
            </Link>
