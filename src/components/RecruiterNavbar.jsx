...
const navLinks = [
  ...
  { to: '/company/Posting', label: 'Job Posting' },
...
const RecruiterNavbar = () => {
  ...
  const initials = (user?.user_metadata?.name || user?.email || 'R').charAt(0).toUpperCase();

  return (
    <nav className={`fixed top-0 left-0 right-0 w-full z-50 px-4 md:px-8 pt-4 pb-0 pointer-events-none transition-transform duration-500 ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>
                ...
                {isProfileOpen && (
                  <div className="absolute right-0 mt-3 w-64 bg-white/90 dark:bg-[#1a1c1a]/95 backdrop-blur-2xl rounded-[28px] shadow-modal border border-white/40 dark:border-white/10 py-3 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
                    <div className="px-6 py-4 border-b border-slate-500/10 mb-2">
                      <p className="font-bold text-slate-900 dark:text-white truncate text-base">{user?.user_metadata?.name || 'Recruiter'}</p>
                      <p className="text-[11px] text-slate-500 font-semibold truncate tracking-wider uppercase opacity-60">{user?.email}</p>
                    </div>
